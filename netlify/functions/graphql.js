...
const resolvers = {
  Query: {
    tasks: (_, args) => {
      ...
      if (args.search) filtered = filtered.filter(t => t.title.toLowerCase().includes(args.search.toLowerCase()));
  ...
  Mutation: {
    createTask: (_, { input }) => {
      const task = { id: String(data.tasks.length + 1), ...input, actualHours: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), watcherIds: input.watcherIds || [] };
    ...
    createComment: (_, { input }) => {
      const comment = { id: String(data.comments.length + 1), ...input, authorId: '1', createdAt: new Date().toISOString(), isEdited: false };
      data.comments.push(comment);
      return comment;
    },
    logTime: (_, { input }) => {
      const entry = { id: String(data.timeEntries.length + 1), userId: '1', ...input };
